...
import fr.metakonsulting.mvc.controler.ControlerTestThread;
...
import fr.metakonsulting.mvc.utilitaires.MsgTrfObjet;
...
public class IHM_Test_Thread extends JFrame implements Constantes, IHM, Serializable {
	...
	/**
	 * numero de serie de la classe : pour la compatibilite en cas de serialisation d'un objet
	 */
	...
	private void btnGo_clic(ActionEvent e) throws InterruptedException {
		...
		if (isclicOnBtnInitAppli) {
			if (isClicOnBtnGO) {
				// si le test est deja lance, on ne fait rien de plus
		...
		controleur.dmdIHMGo();	// lancement du test 
		isClicOnBtnGO = true;
		}
	...
	/**
	 * methode appelee lors du clic sur le bouton "Creer Threads" ou par la methode initAppli()
	 * cette methode s'occupe d'initialiser les composants utilises pour les tests
	 * 
	 * @param e
	 * @throws InterruptedException 
	 */
...
//			JOptionPane.showMessageDialog(null, "Creation des threads : OK");
	...
	private void btnClicTestPoolThread(ActionEvent e) {
		initAppli(e);		// initialisation de l'application
		controleur.dmdIHMLanceTestPool();	// lancement du test demande au controleur
	}
	...
	private void btnClicTestSemaphore(ActionEvent e) {
		...
		// lancement du test demande au controleur
